...
$(function(){
	...
	moduleInit();
    //表单非空验证
    $("#proName,#orderHour,#vipPrice,#originalCost，#proUnit").on("blur",function () {
        validate($(this));
    })
    //保存和保存并提交的按钮监听事件
    ...
    function saveSubmit(_this){
    	...
    	if(!$("#proUnit").val().trim()){
    		$("#proUnit").next().css("display","inline-block");
    		return false;
    	}
        //1.特产类型
		...
		console.log("特产传入后台的参数",arrActionParam);
        var strActionName="Product_AddNativeProduct";
